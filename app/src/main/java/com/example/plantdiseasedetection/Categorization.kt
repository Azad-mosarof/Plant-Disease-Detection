...
class Categorization(assetManager: AssetManager,modelPath: String,labelPath: String, inputSize: Int) {
    ...
    fun recognizeImage(bitmap: Bitmap): List<com.example.plantdiseasedetection.Categorization.Categorization>{
    ...
    private fun getSortedResult(result: Array<FloatArray>): List<com.example.plantdiseasedetection.Categorization.Categorization> {
        val pq = PriorityQueue(
            GREAT_OUTCOME_MXX,
            kotlin.Comparator<com.example.plantdiseasedetection.Categorization.Categorization>{
        ...
        for (i in ROW_LINE.indices){
            ...
            if(confidence >= POINT_THRHLD){
                pq.add(com.example.plantdiseasedetection.Categorization.Categorization(
                    ""+i,
                    if(ROW_LINE.size > i) ROW_LINE[i] else "Unknown", confidence
                ))
            }
        }
        val recognition = ArrayList<com.example.plantdiseasedetection.Categorization.Categorization>()
        val recognitionSize = Math.min(pq.size,GREAT_OUTCOME_MXX)
        for(i in 0 until recognitionSize){
            recognition.add(pq.poll())
        }
        return recognition
    }
